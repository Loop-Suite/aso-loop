...
#[command(
    name = "aso",
    version,
    about = "Claude Code(`claude -p`)를 백엔드로 앱스토어 리스팅 카피(제목/부제/키워드/설명)를 생성하고 루브릭으로 채점하는 CLI"
)]
struct Cli {
    /// claude 실행 파일 경로
...
enum Cmd {
    /// 리스팅 카피 초안 N개 생성 + 채점 + 랭킹 리포트
    Gen {
        #[arg(long)]
        spec: PathBuf,
        /// 앱 개요·기능·타겟 유저 등을 담은 브리프 파일 (md/txt)
        #[arg(long)]
        brief: PathBuf,
...
fn real_main() -> Result<()> {
    ...
    match &cli.cmd {
        Cmd::Gen { spec, brief, count, out, rounds, concurrency, no_score } => {
            ...
            println!("생성 {}건 — {} ({})", count, sp.name, sp.store.label());
            ...
            if failed > 0 {
                eprintln!("경고: 생성 {failed}건 실패 (요청 {requested}건 중 {}건 성공)", docs.len());
            }
        ...
        Cmd::Score { spec, input, out, rounds, concurrency } => {
            ...
            println!("채점 {}건 — {} ({})", files.len(), sp.name, sp.store.label());
        ...
        Cmd::Loop { spec, brief, out, target, max_iter, rounds, min_delta, patience, angle, gate_model } => {
            ...
            println!("\n종료: {} · 최고 {:.1}/100 ({})", r.stop_reason, r.best_score.total, r.best_label);
            for w in &r.warnings {
                println!("  ⚠ {}", w);
            }
...
fn score_many(judges: &[Llm], sp: &Spec, docs: Vec<(String, String)>, rounds: usize, concurrency: usize, out_dir: &Path) -> Vec<Scored> {
    ...
    if failed > 0 {
        eprintln!("경고: 채점 {failed}건 실패 (요청 {requested}건 중 {}건 성공)", scored.len());
    }
    for s in &scored {
